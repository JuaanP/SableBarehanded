...
public class BarehandedConfigScreen {
    ...
    public static Screen create(Screen parent) {
        ...
        builder.setSavingRunnable(() -> {
            ...
            if (mc.player != null && (mc.hasSingleplayerServer() || mc.player.hasPermissions(2))) {
                ...
                dev.juaanp.sablebarehanded.platform.Services.NETWORK.sendUpdateServerConfig(json);
            }
        });
    ...
    private static void buildCoreGrab(ConfigCategory server, ConfigEntryBuilder eb) {
        ...
        addInt(core, eb, "Block Limit", "Maximum number of connected blocks that can be grabbed at once. 0 = unlimited.", 0, 10000, ServerConfig.INSTANCE.blockLimit, SERVER_DEFAULTS.blockLimit, v -> ServerConfig.INSTANCE.blockLimit = v);
    ...
    private static void buildClientCategories(ConfigCategory client, ConfigEntryBuilder eb) {
        buildRotationInput(client, eb);
        buildArmRendering(client, eb);
        buildHud(client, eb);
        buildInteraction(client, eb);
    }

    private static void buildRotationInput(ConfigCategory client, ConfigEntryBuilder eb) {
    ...
    private static void buildArmRendering(ConfigCategory client, ConfigEntryBuilder eb) {
        SubCategoryBuilder armRendering = eb.startSubCategory(Component.literal("Arm Rendering"));

        addBoolean(armRendering, eb, "Hide 1st Person Arms", "Hides custom arms in 1st person.", ClientConfig.INSTANCE.hideFirstPersonArms, CLIENT_DEFAULTS.hideFirstPersonArms, v -> ClientConfig.INSTANCE.hideFirstPersonArms = v);
        addBoolean(armRendering, eb, "Hide 3rd Person Arms", "Hides custom arms in 3rd person.", ClientConfig.INSTANCE.hideThirdPersonArms, CLIENT_DEFAULTS.hideThirdPersonArms, v -> ClientConfig.INSTANCE.hideThirdPersonArms = v);
        ...
        addDouble(armRendering, eb, "Arm Transition Delay", "Fraction of the transition (0.0 - 1.0) before custom arms start rising.", 0.0, 1.0, ClientConfig.INSTANCE.armTransitionDelay, CLIENT_DEFAULTS.armTransitionDelay, v -> ClientConfig.INSTANCE.armTransitionDelay = v);
        addDouble(armRendering, eb, "Arm Vanilla Drop Distance", "Distance (m) the original vanilla arms are pushed down during transition.", 0.0, 10.0, ClientConfig.INSTANCE.armVanillaDropDistance, CLIENT_DEFAULTS.armVanillaDropDistance, v -> ClientConfig.INSTANCE.armVanillaDropDistance = v);
    ...
    private static void buildHud(ConfigCategory client, ConfigEntryBuilder eb) {
        ...
        addBoolean(hud, eb, "Hide Grab HUD", "Hides rotation/mass HUD when grabbing.",
                ClientConfig.INSTANCE.hideGrabHud, CLIENT_DEFAULTS.hideGrabHud,
                v -> ClientConfig.INSTANCE.hideGrabHud = v);

        addBoolean(hud, eb, "Hide Placement Overlay", "Hides permanent Physics Placement HUD overlay.",
                ClientConfig.INSTANCE.hidePhysicsPlacementOverlay, CLIENT_DEFAULTS.hidePhysicsPlacementOverlay,
                v -> ClientConfig.INSTANCE.hidePhysicsPlacementOverlay = v);

        addBoolean(hud, eb, "Show Placement Message", "Shows temporary message when toggling physics placement.",
                ClientConfig.INSTANCE.showPhysicsPlacementMessage, CLIENT_DEFAULTS.showPhysicsPlacementMessage,
                v -> ClientConfig.INSTANCE.showPhysicsPlacementMessage = v);

        client.addEntry(hud.build());
    }

    private static void buildInteraction(ConfigCategory client, ConfigEntryBuilder eb) {
